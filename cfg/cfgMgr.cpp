...
#define APPEND_KEY_USAGE(key, mod, usage) \
    do { keyBinding.emplace(CCfgHelper::getKeyHash(key, mod), usage); } while(0)
namespace CFG {
    ...
    void CCfgMgr::buildCfg_imgView(QDomDocument& doc, QDomElement& root) const
    {
        ...
        CXmlHelper::appendChild(doc, imgView,
            CXmlHelper::TXmlNode{ TAG_DSP_STGY, QString::number(m_cfg.imgView.dspStgy), NO_ATTR, COMMENT_DSP_STGY });

        //dspOrder
        CXmlHelper::appendChild(doc, imgView,
            CXmlHelper::TXmlNode{ TAG_DSP_ORDER, QString::number(m_cfg.imgView.dspOrder), NO_ATTR, COMMENT_DSP_ORDER });

        //keyBinding
        auto keyBinding = doc.createElement(TAG_KEY_BINDING);
        imgView.appendChild(keyBinding);
        const auto& keyUsageDict = m_cfg.imgView.keyUsageDict;
        for (const auto& keyUsage : keyUsageDict)
        {
            appendKeyUsage(doc, keyBinding, keyUsage.first, keyUsage.second);
        }
        return;
    }
